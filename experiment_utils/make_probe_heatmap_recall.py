#!/usr/bin/env python
"""
make_probe_heatmap_recall.py

Create a layer × predicate-family heat-map from the MASKING probes
(trained with linear_probe_LXX.pth).
Plots POSITIVE-CLASS RECALL (for class 1) averaged per category.
This metric focuses on: "Of all actual 'True' instances, how many did the probe find?"
Allows choosing between 'unweighted_recall' and 'support_weighted_recall'.
Assumes category is the FIRST WORD in the label string (space separated).

Usage:
    python make_probe_heatmap_recall.py --metric unweighted_recall --outfile heatmap_masking_unweighted_recall.png
    python make_probe_heatmap_recall.py --metric support_weighted_recall --outfile heatmap_masking_support_weighted_recall.png
"""
...
from sklearn.metrics import recall_score # We'll use recall_score
import warnings

warnings.filterwarnings('ignore', category=UserWarning, module='sklearn')
# --------------------------------------------------------------------------- #
cli = argparse.ArgumentParser(description="Generate Positive-Class Recall heatmap from masking probe checkpoints.")
...
cli.add_argument("--metric", required=True, choices=['unweighted_recall', 'support_weighted_recall'],
                 help="Which recall averaging to plot ('unweighted_recall' or 'support_weighted_recall')")
cli.add_argument("--success_only", action="store_true", help="Use only successful episodes for evaluation")
cli.add_argument("--outfile", default="probe_heatmap_recall.png", help="Output filename for the heatmap image")
...
if not OBJ_KEYS_PATH.exists(): raise FileNotFoundError(f"OBJ_KEYS not found: {OBJ_KEYS_PATH}")
if not ACT_KEYS_PATH.exists(): raise FileNotFoundError(f"ACT_KEYS not found: {ACT_KEYS_PATH}")

OBJ_KEYS = ast.literal_eval(OBJ_KEYS_PATH.read_text())
ACT_KEYS = ast.literal_eval(ACT_KEYS_PATH.read_text())
ALL = OBJ_KEYS + ACT_KEYS

# !!! REVERTED TO SIMPLER cat FUNCTION (assuming space-separated category is first word) !!!
def cat(lbl: str) -> str:
    """Extracts the category part of a label string (assumed to be the first word)."""
    if not isinstance(lbl, str):
        # print(f"Warning: Non-string label encountered: {lbl}. Returning as is.") # Optional print
        return str(lbl)
    try:
        ...
        # print(f"Warning: Label string '{lbl}' could not be split. Returning as is.") # Optional print
        return lbl


CAT_NAMES = sorted({cat(k) for k in ALL})
# Keep the debug prints for your verification
print("-" * 20)
...
print("-" * 20)
if len(CAT_NAMES) > 50 and len(CAT_NAMES) > len(ALL) * 0.8 :
    print("WARNING: Number of categories is very high, close to the total number of labels.")
    print("This might indicate the `cat` function (lbl.split()[0]) is not correctly parsing your label strings into broader families.")
    print("Ensure your label key files have labels like: `category_name detail1 detail2 ...`")
     ...
     for l_idx in range(L_MAX):
         if l_idx in eps[first_ep_key]["visual_semantic_encoding"]:
             if eps[first_ep_key]["visual_semantic_encoding"][l_idx].ndim >=2:
                ...
                break
             else: print(f"Warning: Embedding for ep {first_ep_key}, layer {l_idx} not 2D+.")
if input_dim_ref is None: print("CRITICAL WARNING: Could not determine ref input_dim.")

print(f"\nGenerating heatmap for metric: Positive-Class Recall ({args.metric})")
...
for L in range(L_MAX):
    ...
    for ep_idx, d in eps.items():
        ...
        try:
            ...
            for col_idx, global_label_idx in enumerate(keep_indices):
                ...
                recall_val = recall_score(y_true_col, y_pred_col, labels=[0,1], pos_label=1, average='binary', zero_division=0)
                
                if args.metric == 'support_weighted_recall':
                    per_cat_metrics[category_name].append((recall_val, len(y_true_col)))
                else: per_cat_metrics[category_name].append(recall_val)
    ...
    if num_valid_ep_steps_layer > 0:
        for cat_idx, category_name in enumerate(CAT_NAMES):
            ...
            if metrics_for_cat:
                if args.metric == 'support_weighted_recall':
                    recall_values, supports = zip(*metrics_for_cat)
                    if sum(supports) > 0: heat[L, cat_idx] = np.average(recall_values, weights=supports)
                    else: heat[L, cat_idx] = np.mean(recall_values) if recall_values else np.nan
...
# --------------------------------------------------------------------------- #
# Plotting code remains largely the same
...
metric_title_frag = "Support-Weighted Positive-Class Recall" if args.metric == 'support_weighted_recall' else "Unweighted Positive-Class Recall"
title = f"Validation {metric_title_frag} (True vs False) per layer × category (Masking -1 Probes)"
...
print("\n✅ Heatmap script (Positive-Class Recall for Masking Probes) finished.")